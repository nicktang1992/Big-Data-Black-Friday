...
plotdf = BF[sample(nrow(BF), 100), -c(1,2,9,10,11)]
summary(plotdf)
plot(plotdf)
...
##
## deal with subset of occupation
##
#create subset of data rows by: occupation
occupation_df <- list()
for(i in 1:20) {
...
#if you want deal with other subset of occupation, change it here.
...
testing_df2 = current_df[,-c(1,4)][-indeces2,]

indeces3 = sample(nrow(current_df[,-c(1,4)]), nrow(current_df[,-c(1,4)])*0.7)
training_df3 = current_df[,-c(1,4)][indeces3,]
testing_df3 = current_df[,-c(1,4)][-indeces3,]

#TO-DO: run LM and GLM on all three data sets
#deal with training_df1, if you want to deal with other training subset,change the number. 
...
##
## deal with subset of marital&age
##
marital_a_df=BFTEST[which(BFTEST$Marital_Status==1),]
marital_b_df=BFTEST[which(BFTEST$Marital_Status==0),]
age_m_a_df = list()
age_m_b_df = list()
#divide each subset into 7 subsubset
for(i in 1:7){
  age_m_a_df[[i]]=marital_a_df[which(marital_a_df$Age==(i-1)/6),]
  age_m_b_df[[i]]=marital_b_df[which(marital_b_df$Age==(i-1)/6),]
}
...
age_City_C_df = list()
for(i in 1:7){
  age_City_A_df[[i]]=City_A_df[which(City_A_df$Age==(i-1)/6),]
  age_City_B_df[[i]]=City_B_df[which(City_B_df$Age==(i-1)/6),]
  age_City_C_df[[i]]=City_C_df[which(City_C_df$Age==(i-1)/6),]
}
